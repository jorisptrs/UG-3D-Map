...
var map = new mapboxgl.Map({
   ...
   bearing: -17.6,
   container: 'map',
   antialias: false //set to true for better graphics but worse performance
});
...
map.on('load', function () {
   ...
   let rug = [12575188,1311904,3187586,3187584,3187584,3187580,3187592,3187589,3187585,3187583,3187580,3187593,3187592,3187580,1280431,1280431,3187586,3187591,3187590,3187576,3187576,3187588,3187590,3187582,12728487,3187587,3187586,3154709,3149500,1328605,1328603,1328603,1328604,3165763,3165763,1324385,1324385,1337126,1337128,3204056,12728490,12728488,3156369,1316451,3145488,3145489,1311903,3142763,3165768,3187715,1311905];
   map.addLayer({
      "id": "buildings3D-top",
      "type": "fill-extrusion",
      "metadata": {},
   ...
   //    var features = map.queryRenderedFeatures(e.point).filter((feature) => feature.layer.id === 'buildings3D');
   ...
   window.addEventListener('mousemove', function (e) {
      ...
      const features = map.queryRenderedFeatures(e.point, { layers: ['buildings3D-top'] });
...
async function show() {
   var ugbuildings = await fetchJSON("data/places/places.geojson");
   ugbuildings = ugbuildings.features;
